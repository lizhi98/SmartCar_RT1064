...
static uint8 soft_iic_wait_ack (soft_iic_info_struct *soft_iic_obj)
{
    ...
    gpio_set_dir(*soft_iic_obj->sda_pin, GPI);
#endif
    soft_iic_delay(soft_iic_obj->delay);
    ...
    gpio_low(*soft_iic_obj->scl_pin);                                            // SCL 低电平
#if SOFT_IIC_SDA_IO_SWITCH
    gpio_set_dir(*soft_iic_obj->sda_pin, GPO);
#endif
    soft_iic_delay(soft_iic_obj->delay);
...
{
    assert(NULL != soft_iic_obj);
    assert(NULL != write_data);
    assert(NULL != read_data);
    soft_iic_start(soft_iic_obj);
    soft_iic_send_data(soft_iic_obj, soft_iic_obj->addr << 1);
    while(write_len --)
    {
...
// 备注信息     
//-------------------------------------------------------------------------------------------------------------------
void soft_iic_init (soft_iic_info_struct *soft_iic_obj, uint8 addr, uint32 delay, gpio_struct *scl_pin, gpio_struct *sda_pin)
{
    assert(NULL != soft_iic_obj);
    assert(scl_pin->pin != sda_pin->pin);                                              // 醒醒！ scl_pin 与 sda_pin 怎么能填同一个引脚？
    soft_iic_obj->scl_pin = scl_pin;
    soft_iic_obj->sda_pin = sda_pin;
    soft_iic_obj->addr = addr;
    soft_iic_obj->delay = delay;
    gpio_init(*scl_pin, GPO, GPIO_HIGH, PULL_UP);                          // 提取对应IO索引 AF功能编码
    gpio_init(*sda_pin, GPO, GPIO_HIGH, OPEN_DTAIN);                         // 提取对应IO索引 AF功能编码
}
