...
net_path = "model2.tflite"                                  # 定义模型的路径
labels = [line.rstrip() for line in open("/sd/label.txt")]   # 加载标签
...
while True:
    ...
    # for r in img.find_rects(threshold = 50000):             # 在图像中搜索矩形
        ...
        # 暂定义全部图像为矩形
    re = (0,0, img.width(), img.height())
    img.draw_rectangle(re, color = (255, 0, 0))   # 绘制矩形外框，便于在IDE上查看识别到的矩形位置
    # img1 = img.copy(1, 1, r.rect())                           # 拷贝矩形框内的图像
    img1 = img.copy(1, 1, re)                           # 拷贝矩形框内的图像
